...
class BaseClient{
    constructor(options){
        ...
        this.enpoint = 'wss://gateway.discord.gg/?v=9&encoding=json';
        this.isReady = false;
    }
        ...
        this.ws.on('message', async(data) => {
            ...
            switch (data.t) {
                ...
                default:
                    ...
                    console.warn(`[ERROR] UNKNOWN EVENT ${data.t}`);
                    break;
            }
    ...
    async createUserClient(){
        const data = await axios('https://discordapp.com/api/v9/users/@me', {
            method: 'GET', 
            headers: {
    ...
    async loadChannels(guildid) {
        const data = await axios(`https://discord.com/api/v9/guilds/${guildid}/channels`, {
            method: 'GET',
            headers: {
                'Authorization': this.token
            }
    ...
    async loadRoles(guildid) {
        const data = await axios(`https://discord.com/api/v9/guilds/${guildid}/roles`, {
            method: 'GET',
            headers: {
                'Authorization': this.token
            }
    ...
    async loadGuilds() {
        const data = await axios('https://discord.com/api/v9/users/@me/guilds', {
            method: 'GET',
            headers: {
                'Authorization': this.token
            }
